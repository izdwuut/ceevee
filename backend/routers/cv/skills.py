...
@skills_router.patch(PREFIX + "/{skill_id}", response_model=Skill_Out_Pydantic)
...
@skills_router.delete(PREFIX + '/{skill_id}')
async def delete_skill(skill_id: UUID4) -> None:
    await rest.delete_item(SkillModel, skill_id)


@skills_router.post("/{cv_id}/skill", response_model=Skill_Out_Pydantic)
