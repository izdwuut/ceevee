...
@languages_router.patch(PREFIX + "/{language_id}", response_model=Language_Out_Pydantic)
...
@languages_router.delete(PREFIX + '/{language_id}')
async def delete_language(language_id: UUID4) -> None:
    await rest.delete_item(LanguageModel, language_id)


@languages_router.post("/{cv_id}/language", response_model=Language_Out_Pydantic)
